...
public class GpsWidgetProvider extends AppWidgetProvider{
    ...
    public static String GPS_CHANGED = "com.evervolv.widget.GPS_CLICKED";
	...
	*/
	private void updateWidgetView(Context context,int state){
		...
		views.setTextViewText(R.id.power_label,context.getString(R.string.gps_gadget_caption));
		...
		if (state == StateTracker.STATE_DISABLED){
			...
			views.setImageViewResource(R.id.power_item,R.drawable.widget_gps_icon_off);
		} else if (state == StateTracker.STATE_ENABLED) {
			views.setImageViewResource(R.id.power_trigger,R.drawable.power_switch_on);
			views.setImageViewResource(R.id.power_item,R.drawable.widget_gps_icon_on);
